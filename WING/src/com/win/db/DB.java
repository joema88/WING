...
public class DB {
	...
	public static PreparedStatement getRockInsertStatement() {
		...
		if (rockStmnt == null) {
			try {
				String query = " insert into BBROCK(STOCKID,DATEID,PERCENT,CLOSE,NETCHANGE,ATR,OPEN,HIGH,LOW,LOW52,HIGH52,MARKCAP,VOLUME,YELLOW,TEAL,PINK,BB1,BB2,BB3,BB4)"
						+ " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static void main(String[] args) {
		 ...
		 * (DATEID)); 
		 * 3. CREATE TABLE BBROCK(STOCKID SMALLINT, DATEID SMALLINT, PERCENT
		 * FLOAT, CLOSE FLOAT,NETCHANGE FLOAT, ATR FLOAT, OPEN FLOAT,HIGH FLOAT, LOW
		 * FLOAT,LOW52 FLOAT,HIGH52 FLOAT, MARKCAP FLOAT,VOLUME FLOAT, YELLOW SMALLINT,
		 * TEAL SMALLINT, PINK SMALLINT, BB1 SMALLINT, BB2 SMALLINT, BB3 SMALLINT, BB4
		 * SMALLINT, PRIMARY KEY (STOCKID, DATEID), FOREIGN KEY (STOCKID) REFERENCES
		 * SYMBOLS(STOCKID) ON DELETE CASCADE, FOREIGN KEY (DATEID) REFERENCES
		 * DATES(DATEID));
		 */
		//select a.DATEID,CDATE, a.STOCKID, CLOSE, MARKCAP, VOLUME FROM BBROCK a, SYMBOLS b, DATES c  WHERE a.STOCKID = b.STOCKID and a.DATEID=c.DATEID and b.SYMBOL='BRK/A';
		// $$$$$$$$$ SQL
